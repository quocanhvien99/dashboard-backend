...
export function getdepartment(req: Request, res: Response) {
	...
	const sqlCon: mysql.Connection = req.app.locals.sqlCon;
	sqlCon.query(
		`select D.id as id, dname, U.name as dhead, C.name as cname, C.id as cid from user as U, department as D, course as C where D.id=? and D.dhead_id=U.id and D.id=C.did`,
		[id],
		(err: any, result: any) => {
			if (err) return res.status(400).json(err);
			res.json(result[0]);
